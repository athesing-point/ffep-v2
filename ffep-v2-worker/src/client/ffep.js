class FFEP {
  constructor(options = {}) {
    ...
    this.workerUrl = options.workerUrl || 'https://point.com/api/ffep';
  }
  ...
  async fetchSuggestions(query) {
    const response = await fetch(\`\${this.workerUrl}/suggestions?\${new URLSearchParams({
      q: query
    })}\`);
...
document.addEventListener('DOMContentLoaded', () => {
  const ffep = new FFEP({
    workerUrl: 'https://point.com/api/ffep' // Replace with your worker URL
  });
  ffep.init();
}); 